...
if __name__ == '__main__':
    ...
    while(clock):
        #print(f"Clock: {clock}")
        jobsLeft = getJobsLeft(client_id, session_id) #recieves an integer
        if not jobsLeft:
            break
        response = getJob(client_id,session_id,clock) #recieves a dictionary
        if response and response['success']:
            if response['data']:
                for data in response['data']:
                    job_id = data['job_id'] #Pulls the job id from the dictionary
                    print(f"Job {job_id} received at {clock}...")
                    if job_id not in jobs:
                        jobs[job_id] = {'data':data,'bursts':{}} #Creates a dictionary with the job id as the key
